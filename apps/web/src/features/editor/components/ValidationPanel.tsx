  ...
  if (isLoading) {
    return (
      <div className="p-4 space-y-2 animate-pulse">
        {[1, 2, 3].map(i => <div key={i} className="h-10 bg-editor-border rounded" />)}
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2 border-b border-editor-border">
        ...
        <button onClick={() => refetch()} className="text-xs text-editor-muted hover:text-editor-text">↻</button>
      </div>

      <div className="flex-1 overflow-y-auto">
        {issues.length === 0 ? (
          <p className="p-4 text-xs text-editor-muted text-center">No validation issues found</p>
        ) : (
          <div className="p-2 space-y-1">
            {issues.map((issue, i) => (
              <button
                ...
                className="w-full text-left p-2 rounded text-xs hover:bg-editor-border transition-colors"
              >
                <div className="flex items-start gap-2">
                  <span className={`mt-0.5 flex-shrink-0 ${issue.severity === 'error' ? 'text-red-400' : 'text-yellow-400'}`}>
                    {issue.severity === 'error' ? '●' : '▲'}
                  </span>
                  <span className="text-editor-text leading-relaxed">{issue.message}</span>
                </div>
                {issue.time !== undefined && (
                  <span className="ml-5 text-editor-muted">Jump to {issue.time}s →</span>
                )}
              </button>
            ))}
          </div>
